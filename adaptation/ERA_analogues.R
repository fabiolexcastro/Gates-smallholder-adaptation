...
#soldir <- "C:/Datasets/SoilGrids"
#hzdir <- "C:/Datasets/Adaptation Atlas Hazards/"
...
#load Soils Data -----
if(DoLite){
  load(paste(cimdir,"/soils2.rda",sep=""))
}else{
...
for(k in 1:nrow(Vars)){
    ...
    #climate: load future ####
    File<-paste0(cimdir,"/prec_",gsub("[.]","_",Scenario),"_",Year,"_","Resamp.RData")
    if(!file.exists(File)){
    ...
    File<-paste0(cimdir,"/tmean_",gsub("[.]","_",Scenario),"_",Year,"_","Resamp.RData")
    if(!file.exists(File)){
