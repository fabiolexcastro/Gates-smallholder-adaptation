...
suitCalc <- function(climPath='', sowDat='', harDat='', Gmin=90,Gmax=90,Tkmp=0,Tmin=9,Topmin=20,Topmax=34,Tmax=50,Rmin=150,Ropmin=300,Ropmax=400,Rmax=600, outfolder='', cropname='',ext="",cropClimate=F) {
  ...
  cat("\n")
  #pbClose(pb)
  pSuitRaster <- writeRaster(pSuitRaster, pSuitName, overwrite=TRUE)
  tSuitRaster <- writeRaster(tSuitRaster, tSuitName, overwrite=TRUE)
  fSuitRaster <- writeRaster(fSuitRaster, fSuitName, overwrite=TRUE)
  cumPptRaster <- writeRaster(cumPptRaster, cumPptName, overwrite=TRUE)
  meanTRaster <- writeRaster(meanTRaster, meanTName, overwrite=TRUE)
  gsSuitRaster <- writeRaster(gsSuitRaster, gsSuitName, overwrite=TRUE)
